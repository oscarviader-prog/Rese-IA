...
import { ArrowLeft, Search, ShieldCheck, Building2, User, LogIn, UserPlus, LogOut } from 'lucide-react';
            ...
            <div className="flex items-center gap-2 pl-2 border-l border-white/15">
              <div className="text-right hidden md:block">
                <p className="text-xs font-bold text-white truncate max-w-[120px]">
                  {user.firstName || user.companyName || user.email.split('@')[0]}
                </p>
                <span className="text-[10px] font-mono-code text-[#00f2ff] block uppercase">
                  {user.role}
                </span>
              </div>
              <button
                onClick={logout}
                title="Cerrar sesión"
                className="p-1.5 rounded-lg bg-slate-900 text-slate-300 hover:text-white border border-white/20 hover:bg-slate-800 transition-colors"
              >
